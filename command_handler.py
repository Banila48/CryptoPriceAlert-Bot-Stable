...
class CommandHandler:
    ...
    def higher_lower(self, chatId, command):
        ...
        msg = '⏳ I will send you a notification when the price of {} is {} {} {}.'.format(
...
##        self.api.sendMessage(resp, chatId, "Markdown")
        self.api.sendMessage(resp, chatId, parse_mode=ParseMode.HTML)
                ...
                for op in alerts[fsym]:
                    for tsym in alerts[fsym][op]:
                        for target in alerts[fsym][op][tsym]:
                            price = self.repository.get_price_if_valid(fsym, tsym)
                            msg='{}{} {} <b>{}</b> {}. Current Price is <i>{}</i>\n\n'.format(msg, self.repository.get_symbols()[fsym], op, target,tsym, price)
            self.api.sendMessage(msg, chatId, parse_mode=ParseMode.HTML)
        else:
